...
func (ev *evaluator) aggregation(op parser.ItemType, grouping []string, without bool, param interface{}, vec Vector, seriesHelper []EvalSeriesHelper, enh *EvalNodeHelper) Vector {
	...
	for si, s := range vec {
		...
		if op == parser.COUNT_VALUES {
			lb.Reset(metric)
			lb.Set(valueLabel, strconv.FormatFloat(s.F, 'f', -1, 64))
		...
		case parser.TOPK:
			if int64(len(group.heap)) < k || group.heap[0].F < s.F || math.IsNaN(group.heap[0].F) {
		...
		case parser.BOTTOMK:
			if int64(len(group.reverseHeap)) < k || group.reverseHeap[0].F > s.F || math.IsNaN(group.reverseHeap[0].F) {
